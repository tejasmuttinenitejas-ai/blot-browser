...
export default function BookmarksScreen({ navigation }) {
  ...
  useFocusEffect(useCallback(() => { getBookmarks().then(setBookmarks); }, []));
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => Alert.alert('Clear', 'Remove all bookmarks?', [{ text: 'Cancel', style: 'cancel' }, { text: 'Clear', style: 'destructive', onPress: async () => { await clearBookmarks(); setBookmarks([]); } }])} style={styles.headerBtn}>
      ...
      {bookmarks.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.row} onPress={() => navigation.navigate('Browser', { url: item.url })}>
              ...
              <TouchableOpacity onPress={() => removeBookmark(item.url).then(() => setBookmarks(prev => prev.filter(b => b.url !== item.url)))} style={{ padding: 6 }}>
