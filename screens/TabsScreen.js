  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Text style={styles.title}>Blot <Text style={{ color: colors.accent }}>Browser</Text></Text>
        <Text style={styles.subtitle}>Your powerful private browser</Text>
        <View style={styles.grid}>
          {SHORTCUTS.map((s) => (
            <TouchableOpacity key={s.name} style={styles.tile} onPress={() => navigation.navigate('Browser', { url: s.url })}>
              <View style={styles.tileIcon}>{s.icon}</View>
              <Text style={styles.tileLabel}>{s.name}</Text>
            </TouchableOpacity>
          ))}
        </View>
        <TouchableOpacity style={styles.beckyCard} onPress={() => navigation.navigate('Becky')}>
...
const styles = StyleSheet.create({
  ...
  logoMark: { width: 56, height: 56, borderRadius: 16, backgroundColor: colors.accent, alignItems: 'center', justifyContent: 'center', marginBottom: 14 },
  ...
  tile: { width: '28%', alignItems: 'center', gap: 8 },
  tileIcon: { width: 52, height: 52, borderRadius: 14, backgroundColor: colors.surfaceAlt, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border, alignItems: 'center', justifyContent: 'center' },
  tileLabel: { color: colors.textSecondary, fontSize: 11 },
  beckyCard: { flexDirection: 'row', alignItems: 'center', gap: 12, width: '100%', marginTop: 36, backgroundColor: colors.surfaceAlt, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border, borderRadius: 14, padding: 14 },
