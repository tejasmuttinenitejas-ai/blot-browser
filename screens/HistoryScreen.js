  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {history.length === 0 ? (
        ...
        <FlatList
          data={groupByDay(history)}
          keyExtractor={(s) => s.date}
          renderItem={({ item: section }) => (
            <View>
              <Text style={styles.sectionHeader}>{section.date}</Text>
              {section.items.map((entry) => (
                <TouchableOpacity key={`${entry.url}-${entry.timestamp}`} style={styles.row}
                  onPress={() => navigation.navigate('Browser', { url: entry.url })}
                  onLongPress={() => removeHistoryEntry(entry.url).then(() => setHistory(prev => prev.filter(h => h.url !== entry.url)))}>
                  <Ionicons name="globe-outline" size={18} color={colors.textSecondary} />
                  <View style={styles.rowText}>
                    <Text style={styles.rowTitle} numberOfLines={1}>{entry.title || entry.url}</Text>
                    <Text style={styles.rowUrl} numberOfLines={1}>{entry.url}</Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  sectionHeader: { color: colors.textTertiary, fontSize: 12, fontWeight: '600', textTransform: 'uppercase', paddingHorizontal: 16, paddingTop: 16, paddingBottom: 6 },
