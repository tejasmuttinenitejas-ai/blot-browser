...
export default function SettingsScreen({ navigation }) {
  ...
  const maskedKey =
    GROQ_API_KEY && GROQ_API_KEY !== 'PASTE_YOUR_GROQ_KEY_HERE'
      ? `${GROQ_API_KEY.slice(0, 6)}${'•'.repeat(20)}`
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <Row
          ...
          subtitle="Used for address bar text searches"
          right={<Text style={styles.valueText}>Google</Text>}
        />

        <SectionLabel>Home page</SectionLabel>
        <View style={styles.inputBlock}>
          <Text style={styles.rowTitle}>Home URL</Text>
          <Text style={styles.rowSubtitle}>Shown when you tap the home button</Text>
          <TextInput
            style={styles.textInput}
            value={homeUrlDraft}
            onChangeText={setHomeUrlDraft}
            onBlur={() => persist({ homeUrl: homeUrlDraft })}
            autoCapitalize="none"
            autoCorrect={false}
        ...
        <SectionLabel>Becky AI</SectionLabel>
        <Row title="Groq API key" subtitle="Set in config.js" right={<Text style={styles.valueText}>{maskedKey}</Text>} />
        <Row title="AI model" subtitle="Groq model in use" right={<Text style={styles.valueText}>{GROQ_MODEL}</Text>} />

        <SectionLabel>Privacy</SectionLabel>
        <Row
          title="Save browsing history"
          subtitle="Keep a record of visited pages"
          right={
            <Switch
              value={settings.saveHistory}
              onValueChange={(v) => persist({ saveHistory: v })}
        ...
        <Row
          ...
          subtitle="Basic protection against tracking scripts"
          right={
            <Switch
              value={settings.blockTrackers}
              onValueChange={(v) => persist({ blockTrackers: v })}
              trackColor={{ true: colors.accent, false: colors.border }}
              thumbColor="#fff"
            />
          }
        ...
        <Row
          ...
          right={
            <TouchableOpacity
              style={styles.dangerBtn}
              onPress={() =>
                Alert.alert('Clear bookmarks', 'Are you sure?', [
                  { text: 'Cancel', style: 'cancel' },
                  { text: 'Clear', style: 'destructive', onPress: clearBookmarks },
                ])
              }
            >
              <Text style={styles.dangerBtnText}>Clear Bookmarks</Text>
            </TouchableOpacity>
          }
        />
        <Row
          ...
          right={
            <TouchableOpacity
              style={styles.dangerBtn}
              onPress={() =>
                Alert.alert('Reset everything', 'This clears settings, history, and bookmarks.', [
                  { text: 'Cancel', style: 'cancel' },
                  {
                    text: 'Reset',
                    style: 'destructive',
